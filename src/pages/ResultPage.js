import { jsx as _jsx, jsxs as _jsxs } from "react/jsx-runtime";
import { useState } from 'react';
import Background from '@/components/ui/Background';
import Footer from '@/components/ui/Footer';
import AppCard from '@/components/features/AppCard';
import IndexAnalysis from '@/components/features/IndexAnalysis';
import Keywords from '@/components/features/Keywords';
function toRisk(spam) {
    if (spam >= 4)
        return '광고와 실제 게임이 완전히 달라요';
    if (spam >= 3)
        return '광고와 다소 차이가 있을 수 있어요';
    if (spam >= 1.5)
        return '광고와 실제가 대체로 일치해요';
    return '광고에 과장이 없는 앱이에요';
}
function buildKeywords(trust) {
    const entries = Object.entries(trust).sort((a, b) => b[1] - a[1]);
    const maxCount = entries[0]?.[1] ?? 1;
    const positions = [
        [22, 18], [58, 14], [76, 30], [10, 38], [42, 32],
        [65, 52], [28, 58], [50, 68], [15, 72], [80, 65],
        [38, 82], [68, 78], [5, 55], [90, 45],
    ];
    return entries.map(([text, count], i) => ({
        text,
        sentiment: 'negative',
        size: Math.round(18 + (count / maxCount) * 32),
        x: positions[i % positions.length][0],
        y: positions[i % positions.length][1],
        dim: i >= 5,
    }));
}
function buildReviews(result) {
    const neg = result.reviews.negative.slice(0, 3).map((r) => ({
        keyword: '부정 리뷰',
        sentiment: 'negative',
        text: r.text,
    }));
    const pos = result.reviews.positive.slice(0, 2).map((r) => ({
        keyword: '긍정 리뷰',
        sentiment: 'positive',
        text: r.text,
    }));
    return [...neg, ...pos];
}
export default function ResultPage({ result, onBack }) {
    const [playKey] = useState(0);
    const app = {
        initial: result.app.name.charAt(0),
        name: result.app.name,
        categories: [result.app.category, result.app.developer].filter(Boolean),
        icon_url: result.app.icon_url,
    };
    const negPct = Math.round(result.signal_scores.avg_rating_score);
    const polPct = Math.round(result.signal_scores.polarization_score);
    const ratingRiskPct = Math.round(result.signal_scores.negative_keyword_score);
    const patternPct = Math.round(result.signal_scores.review_ratio_score);
    const indexData = {
        score: result.spam_score,
        outOf: 5,
        warning: toRisk(result.spam_score),
        bars: [
            { id: 'neg', label: '부정 리뷰 비율', pct: negPct, color: '#ec4f8d', trackGradient: 'linear-gradient(90deg,#f06aa1,#ec4f8d)' },
            { id: 'pol', label: '리뷰 분극화 지수', pct: polPct, color: '#e98246', trackGradient: 'linear-gradient(90deg,#f0995e,#e98246)' },
            { id: 'rat', label: '구글플레이 저평점 비율', pct: ratingRiskPct, color: '#84CC16', trackGradient: 'linear-gradient(90deg,#9ddb33,#84CC16)' },
            { id: 'pat', label: '개발사 양산형 패턴', pct: patternPct, color: '#5cc0b9', trackGradient: 'linear-gradient(90deg,#7ed4cd,#5cc0b9)' },
        ],
    };
    const keywords = buildKeywords(result.review_stats.trust_keywords);
    const reviews = buildReviews(result);
    return (_jsxs("div", { className: "relative flex min-h-screen flex-col", children: [_jsx(Background, {}), _jsx("header", { className: "relative z-2 flex items-center justify-between px-8 py-5", children: _jsxs("button", { type: "button", onClick: onBack, className: "font-product select-none text-[28px] font-bold leading-none tracking-tight transition hover:opacity-80", children: [_jsx("span", { style: { color: '#84CC16' }, children: "De" }), _jsx("span", { style: { color: '#D4D4D4' }, children: "Clone" })] }) }), _jsxs("main", { className: "relative z-2 mx-auto w-full max-w-225 flex-1 px-6 pb-20 pt-4", children: [_jsx(AppCard, { app: app }), _jsx(IndexAnalysis, { data: indexData, playKey: playKey }), keywords.length > 0 && _jsx(Keywords, { keywords: keywords, reviews: reviews })] }), _jsx(Footer, {})] }));
}
